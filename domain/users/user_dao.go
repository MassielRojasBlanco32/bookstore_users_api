...
import (
	...
	"github.com/MassielRojasBlanco32/bookstore_users_api/datasources/mysql/users_db"
	"github.com/MassielRojasBlanco32/bookstore_users_api/utils/mysql_utils"
	"github.com/MassielRojasBlanco32/bookstore_utils-go/logger"
	"github.com/MassielRojasBlanco32/bookstore_utils-go/rest_errors"
)
...
func (user *User) FindByStatus(status string) ([]User, rest_errors.RestErr) {
	...
	defer rows.Close() // Use the defer only when you know you have a valid result

	results := make([]User, 0)
	for rows.Next() {
		var user User
		// Always use a pointer with scan, take the actual field and not a copy
